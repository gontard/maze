...
impl MazeGenerator for RecursiveBacktracker {
    fn generate(
        ...
        let MazeSetup {
            ...
            mut grid,
            mut rng,
        } = init_maze(width, height, seed, start);
    ...
    fn generate(
        ...
        for ((ax, ay), (bx, by)) in walls {
            if uf.union(cell_id(ax, ay), cell_id(bx, by)) {
                ...
                let grid_bx = bx * 2 + 1;
                let grid_by = by * 2 + 1;
                grid[grid_ay][grid_ax] = Tile::Path;
                grid[grid_by][grid_bx] = Tile::Path;
                grid[(grid_ay + grid_by) / 2][(grid_ax + grid_bx) / 2] = Tile::Path;
            }
        }
...
mod tests {
    ...
    macro_rules! generator_tests {
        ($mod_name:ident, $gen:expr) => {
            mod $mod_name {
                use super::*;

                #[test]
                fn correct_dimensions() {
                    let maze = $gen.generate(21, 21, Some(42), None);
                ...
                fn has_exactly_one_start_and_one_exit() {
                    let mut maze = $gen.generate(21, 21, Some(42), None);
                    maze.place_exit();
                    let mut starts = 0;
                    let mut exits = 0;
                    ...
                    for y in 0..maze.height {
                        for x in 0..maze.width {
                            if let Some(d) = distances[y][x] {
                                assert!(
                                    d <= exit_dist,
                                    "cell ({x}, {y}) at distance {d} is farther than exit at {exit_dist}"
                                );
                            }
                        }
                    }
                ...
                fn same_seed_produces_identical_maze() {
                    let maze1 = $gen.generate(21, 21, Some(123), None);
                    let maze2 = $gen.generate(21, 21, Some(123), None);
                ...
                fn different_seeds_produce_different_mazes() {
                    let maze1 = $gen.generate(21, 21, Some(1), None);
                    let maze2 = $gen.generate(21, 21, Some(2), None);
                    assert_ne!(maze1.grid, maze2.grid);
                }

                #[test]
                fn even_dimensions_rounded_to_odd() {
                    let maze = $gen.generate(20, 20, Some(42), None);
                    assert_eq!(maze.width, 21);
                    assert_eq!(maze.height, 21);
                }
                ...
                fn custom_start_position() {
                    let maze = $gen.generate(21, 21, Some(42), Some((5, 5)));
                ...
                fn custom_start_maze_is_solvable() {
                    let mut maze = $gen.generate(21, 21, Some(42), Some((5, 5)));
                    maze.place_exit();
                    assert!(
                        bfs_reachable(&maze, maze.start, maze.exit),
                ...
                fn same_seed_and_start_produces_identical_maze() {
                    let maze1 = $gen.generate(21, 21, Some(123), Some((3, 7)));
                    let maze2 = $gen.generate(21, 21, Some(123), Some((3, 7)));
                    assert_eq!(maze1.grid, maze2.grid);
                    assert_eq!(maze1.start, maze2.start);
                    assert_eq!(maze1.exit, maze2.exit);
                }
